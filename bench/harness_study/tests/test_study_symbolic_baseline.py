...
class SymbolicDesignTests(unittest.TestCase):
    def test_symbolic_identity_binds_arms_halts_bounds_and_thresholds(self):
        ...
        self.assertEqual(len(payload["runner_changes"]), 6)
        self.assertEqual(payload["sealed_predecessors"], list(evolution.SEALED_PREDECESSORS) + [BASELINE_BUILD])
        self.assertEqual(len(payload["sealed_predecessors"]), 5)
