...
class CacheTests(unittest.TestCase):
    ...
    def test_batch_uses_shared_cache_without_changing_selection(self):
        service, calls = self.make_service()
        service.score(5)
        jobs = [(Ruleset("a", 1, 3), 5), (Ruleset("a", 2, 4), 5),
                (Ruleset("b", 2, 9), 5), (Ruleset("a", 2, 4), 5),
                (Ruleset("a", 2, 4), 0), (Ruleset("a", 2, 4), 0)]
        self.assertEqual(service.score_many(iter(jobs)), [15, 20, 45, 20, 0, 0])
        self.assertEqual(len(calls), 4)
        self.assertEqual(service.score(5), 15)
        self.assertEqual(len(calls), 4)
        self.assertEqual(service.score_many([]), [])
