...
/// The reasoner's footprint, recovered from the reification provenance: the
/// inferred triples it materialized into the data graph, and the provenance
/// quads (activities + reifiers) that tag them.
struct ReasonerInferences {
    data_quads: Vec<Quad>,
    provenance_quads: Vec<Quad>,
}

/// Enumerate everything previously materialized by the reasoner for `data_graph_iri`.
/// Inferred triples are recovered from the reifiers whose activity is
/// `prov:wasAssociatedWith` the reasoner agent. Read-only.
fn collect_reasoner_inferences(
    store: &Store,
    data_graph_iri: &str,
) -> anyhow::Result<ReasonerInferences> {
    ...
    let mut data_quads: Vec<Quad> = Vec::new();
    let mut provenance_quads: Vec<Quad> = Vec::new();
    ...
    Ok(ReasonerInferences {
        data_quads,
        provenance_quads,
    })
}

/// The data-graph quads previously materialized by the reasoner into
/// `data_graph_iri`. Used by the canonical export to keep derived edges out of
/// the committed `kg.nq` — imported without their reification tags they would
/// become un-droppable pseudo-asserted edges on the importing machine.
pub fn reasoner_inferred_data_quads(
    store: &Store,
    data_graph_iri: &str,
) -> anyhow::Result<Vec<Quad>> {
    Ok(collect_reasoner_inferences(store, data_graph_iri)?.data_quads)
}

/// Remove every triple previously materialized by the reasoner from `data_graph_iri`,
/// along with its reification provenance in the provenance graph. Idempotent (a no-op
/// when nothing was materialized). Returns the count of data-graph triples removed.
pub fn clear_reasoner_inferences(store: &Store, data_graph_iri: &str) -> anyhow::Result<usize> {
    let ReasonerInferences {
        data_quads: data_removals,
        provenance_quads: prov_removals,
    } = collect_reasoner_inferences(store, data_graph_iri)?;
