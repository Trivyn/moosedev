...
pub struct ProposalSummary {
    ...
    /// Controlled audit reason stored on the replacement rationale.
    pub supersession_reason: Option<String>,
    /// Bounded old/new claim diff generated by the backend.
    pub claim_diff: Option<super::lifecycle::ClaimDiff>,
}
...
fn relation_objects(
    ...
    let mut objects = state
        .store
        .quads_for_pattern(
            Some(NamedNodeRef::new(subject_iri)?.into()),
            Some(NamedNodeRef::new(predicate_iri)?),
            None,
            Some(GraphNameRef::NamedNode(graph)),
        )
        .filter_map(|quad| match quad {
            Ok(quad) => match quad.object {
                Term::NamedNode(object) => Some(Ok(object.as_str().to_string())),
                _ => None,
            },
...
fn scan_record_proposals(state: &AppState) -> anyhow::Result<Vec<ProposalSummary>> {
    ...
    for quad in state.store.quads_for_pattern(
        ...
        let predecessor_iri = relation_objects(state, &iri, &supersedes)?
            .into_iter()
            .next();
        let rationale_iri = relation_objects(state, &iri, &has_rationale)?
            .into_iter()
            .next();
        let predecessor_title = predecessor_iri.as_deref().and_then(|predecessor| {
            first_literal(&state.store, predecessor, &state.capture.title)
                .or_else(|| first_literal(&state.store, predecessor, moose::RDFS_LABEL))
        });
        let supersession_reason = rationale_iri.as_deref().and_then(|rationale| {
            first_literal(
                &state.store,
                rationale,
                super::lifecycle::SUPERSESSION_REASON_PREDICATE,
            )
        });
        let new_properties = [
            (
                state.capture.description.clone(),
                first_literal(&state.store, &iri, &state.capture.description).unwrap_or_default(),
            ),
            (
                state.capture.title.clone(),
                first_literal(&state.store, &iri, &state.capture.title).unwrap_or_default(),
            ),
        ];
        let claim_diff = predecessor_iri.as_deref().and_then(|predecessor| {
            NamedNode::new(predecessor)
                .ok()
                .map(|old| super::lifecycle::claim_diff(state, &old, &new_properties))
        });
        out.push(ProposalSummary {
...
pub fn accept_proposal(
    ...
    match require_pending(state, proposal_iri, &terms)? {
        ...
        ProposalKind::Record => {
            if accept_proposed_supersession(state, proposal_iri, agent)? {
                stamp_resolver(state, proposal_iri, agent);
                state.note_project_write();
                return Ok(AcceptOutcome::Record {
...
/// Atomically ratify a proposed supersession. Returns `false` for an ordinary
/// proposed record, which follows the legacy one-status-flip path.
fn accept_proposed_supersession(
    state: &AppState,
    replacement_iri: &str,
    agent: &str,
) -> anyhow::Result<bool> {
    let supersedes = state.resolve_object_property("supersedes")?;
    let has_rationale = state.resolve_object_property("hasRationale")?;
    let predecessors = relation_objects(state, replacement_iri, &supersedes)?;
    if predecessors.is_empty() {
        return Ok(false);
    }
    anyhow::ensure!(
        predecessors.len() == 1,
        "proposed supersession {replacement_iri} has {} predecessors; expected one",
        predecessors.len()
    );
    let rationales = relation_objects(state, replacement_iri, &has_rationale)?;
    anyhow::ensure!(
        rationales.len() == 1,
        "proposed supersession {replacement_iri} has {} rationales; expected one",
        rationales.len()
    );
    let rationale_status =
        first_literal(&state.store, &rationales[0], &state.capture.status).unwrap_or_default();
    anyhow::ensure!(
        rationale_status == PROPOSED,
        "proposed supersession {replacement_iri} is stale because its rationale is {rationale_status}"
    );

    let predecessor_iri = &predecessors[0];
    let predecessor_status =
        first_literal(&state.store, predecessor_iri, &state.capture.status).unwrap_or_default();
    anyhow::ensure!(
        predecessor_status.is_empty() || super::lifecycle::in_working_set(&predecessor_status),
        "cannot accept stale supersession {replacement_iri}: predecessor {predecessor_iri} is {predecessor_status}"
    );
    let replacements = [
        status_replacement(state, replacement_iri, ACCEPTED)?,
        status_replacement(state, &rationales[0], ACCEPTED)?,
        status_replacement(state, predecessor_iri, "superseded")?,
    ];

    let mut txn = state
        .store
        .start_transaction()
        .map_err(|e| anyhow::anyhow!("accept supersession transaction: {e}"))?;
    for (old_quads, replacement_quad) in &replacements {
        for quad in old_quads {
            txn.remove(quad.as_ref());
        }
        txn.insert(replacement_quad.as_ref());
    }
    txn.commit()
        .map_err(|e| anyhow::anyhow!("accept supersession commit: {e}"))?;
    state.entity_index.invalidate_graph(PROJECT_KG_GRAPH_IRI);
    state.mark_inferred_stale();
    stamp_resolver(state, &rationales[0], agent);
    Ok(true)
}
...
fn status_replacement(
    ...
    subject_iri: &str,
    status: &str,
) -> anyhow::Result<(Vec<Quad>, Quad)> {
    let graph = NamedNodeRef::new(PROJECT_KG_GRAPH_IRI)?;
    let subject = NamedNode::new(subject_iri)?;
    let predicate = NamedNode::new(&state.capture.status)?;
    let old = state
        .store
        .quads_for_pattern(
...
pub fn reject_proposal(state: &AppState, proposal_iri: &str, agent: &str) -> anyhow::Result<()> {
    ...
    if kind == ProposalKind::Record {
        let mut rejected_iris = vec![proposal_iri.to_string()];
        let supersedes = state.resolve_object_property("supersedes")?;
        if !relation_objects(state, proposal_iri, &supersedes)?.is_empty() {
            let has_rationale = state.resolve_object_property("hasRationale")?;
            let rationales = relation_objects(state, proposal_iri, &has_rationale)?;
            anyhow::ensure!(
                rationales.len() == 1,
                "proposed supersession {proposal_iri} has {} rationales; expected one",
                rationales.len()
            );
            rejected_iris.push(rationales[0].clone());
        }
        rejected_iris.extend(
            scan_link_proposals(state, Some(PROPOSED))?
                .into_iter()
                .filter(|dependent| {
                    dependent.kind == ProposalKind::Link && dependent.subject_iri == proposal_iri
                })
                .map(|dependent| dependent.iri),
        );
        let replacements = rejected_iris
            .iter()
            .map(|iri| status_replacement(state, iri, REJECTED))
            .collect::<anyhow::Result<Vec<_>>>()?;
        let mut txn = state
            .store
            .start_transaction()
            .map_err(|e| anyhow::anyhow!("reject record proposal transaction: {e}"))?;
        for (old_quads, replacement) in &replacements {
            for quad in old_quads {
                txn.remove(quad.as_ref());
            }
            txn.insert(replacement.as_ref());
        ...
        txn.commit()
            .map_err(|e| anyhow::anyhow!("reject record proposal commit: {e}"))?;
        state.entity_index.invalidate_graph(PROJECT_KG_GRAPH_IRI);
        for iri in &rejected_iris {
