...
//! typed edges the graph already holds: accepted Constraints on the components
//! that code belongs to, the records those records are motivated by, the
//! current heads of supersession chains, and the Lessons learned from them.
...
/// Constraints rendered with complete claims, counted across every hop; later
/// Constraints render their header only. A Constraint is never dropped.
const CONSTRAINT_CLAIM_LIMIT: usize = 24;
...
pub fn linked_evidence(state: &AppState, files: &[String]) -> anyhow::Result<LinkedEvidence> {
    ...
    for component in &components {
        ...
        let mut constraints: Vec<RecordSummary> =
            collect_records(state, &pairs.component, component)?
                .into_iter()
...
/// then one line counting what the caps left out. Every Constraint is listed;
/// past [`CONSTRAINT_CLAIM_LIMIT`] a Constraint renders without its claim. Other
...
pub fn render_linked_evidence(records: &[LinkedRecord]) -> String {
    ...
    for record in records {
        let is_constraint = record.kind == "Constraint";
        let in_full = if is_constraint {
            constraints += 1;
            constraints <= CONSTRAINT_CLAIM_LIMIT
        } else {
    ...
    fn selection_orders_hops_keeps_the_first_reach_and_drops_dossier_records() {
        ...
        let selected = select(
            vec![
                ...
                candidate("urn:np7", "Constraint", Hop::ComponentConstraint, "Billing"),
                candidate("urn:need", "Requirement", Hop::Motivating, "Decision"),
                candidate("urn:np7", "Constraint", Hop::Motivating, "Decision"),
                candidate("urn:np7", "Constraint", Hop::ComponentConstraint, "Fees"),
                candidate(
                    "urn:head",
        ...
        assert_eq!(
            reached,
            vec![
                ("urn:np7", Hop::ComponentConstraint, "Billing"),
