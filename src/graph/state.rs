...
fn initial_project_root(data_dir: &Path) -> PathBuf {
...
pub struct AppState {
    ...
    /// Canonical repository identity used by project-owned file features.
...
impl AppState {
    ...
    pub fn load_substrate(&self, repo_root: &Path) {
        ...
        if canonical_root.parent().is_none() {
            tracing::warn!(
                "MOOSEDev: refusing filesystem root as project root: {}",
        ...
        *self
            .substrate_repo_root
            .write()
            .unwrap_or_else(|poisoned| poisoned.into_inner()) = Some(canonical_root.clone());
        *self
            .project_root
            .write()
            .unwrap_or_else(|poisoned| poisoned.into_inner()) = canonical_root;
    ...
    /// Canonical project root configured for this state. Production substrate
    /// configuration wins; bootstrap uses the shared project-root resolver.
...
mod tests {
    ...
    #[test]
    fn bootstrap_project_root_never_climbs_above_its_data_anchor() {
        let outer = std::env::temp_dir().join(format!(
            "moosedev-root-anchor-{}-{}",
            std::process::id(),
            chrono::Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
        let nested = outer.join("nested");
        let conventional = nested.join(".moosedev");
        let custom = nested.join("custom-cache");
        std::fs::create_dir_all(&conventional).unwrap();
        std::fs::create_dir_all(&custom).unwrap();
        std::fs::write(outer.join("Cargo.toml"), "[package]").unwrap();

        assert_eq!(
            initial_project_root(&conventional),
            nested.canonicalize().unwrap()
        );
        assert_eq!(
            initial_project_root(&custom),
